...
class Editor:
	def __init__(self, land_tiles, switch, audio):
		...
		# clouds
		self.current_clouds = []
		self.cloud_surf = import_folder(path.join(script_directory, 'graphics', 'clouds'))
	...
	def imports(self):
		...
		self.sky_handle_surf = load(path.join(script_directory, 'graphics', 'cursors', 'handle.png')).convert_alpha()
